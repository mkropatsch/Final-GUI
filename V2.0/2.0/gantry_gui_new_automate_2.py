...
class StepperControlBoard:
    ...
    def send_gcode_with_reply(self, cmd: str, wait_s: float = 0.4) -> list[str]:
        """Send a line and collect reply lines for a short window."""
        self._send_line(cmd)
        t0 = time.monotonic()
        out: list[str] = []
                    ...
                    if cmd:
                        try:
                            ...
                            if hasattr(board, "send_gcode_with_reply"):
                                ...
                                for line in replies:
                                    self._send_message("info", f"[GCODE] {line}")
                            else:

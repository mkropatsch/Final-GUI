...
LOG_PATH = os.path.join(os.path.dirname(__file__), "gantry_messages.log")
...
def parse_full_command(line: str) -> List[Dict[str, Any]]:
    ...
    # Stop grid (new)
    if cmd in ("stopgrid", "gridstop", "stop"):
        msgs.append({"__special__": "stopgrid"})
        return msgs
...
class GantryEngine:
    ...
    def __init__(self, simulate: bool = True):
        ...
        #### NEW ####
        self._grid_stop = threading.Event()
        ...
        while not self._stop_flag:
            ...
            if typ == "state":
                ...
                if level in ("WARN", "WARNING", "ERROR"):
                    #print(f"[MSG: {level}] {text}")
                    log_line(f"[MSG:{level}] {text}")
                    # optionally keep printing warnings/errors only:
                    if level in ("WARN", "WARNING", "ERROR"):
                        print(f"[MSG: {level}] {text}")
    ...
    def send_msg(self, msg: Dict[str, Any]) -> None:
        ...
        if special == "zsteps":
            ...
            # If grid is already running, don't start another
            if self._grid_thread is not None and self._grid_thread.is_alive():
                print("[GRID] Already running. Type 'stopgrid' to stop it.")
                return
            
            self._grid_stop.clear()
            ...
            def _worker():
                self._handle_grid(
                    ...
                    stop_event=self._grid_stop, ### New thing 
                )
            self._grid_thread = threading.Thread(target=_worker, daemon=True)
            self._grid_thread.start()
        
        elif special == "stopgrid":
            self._grid_stop.set()
            print("[GRID] Stop requested.")
    ...
    def _handle_grid(
        ...
        total_s: float,
        serpentine: bool,
        stop_event: Optional[threading.Event] = None, ## New addition
    ) -> None:
        """
...
def print_help() -> None:
    print(
        ...
        "  help                      - show this help\n"
        "  quit / exit               - leave\n"
        "  stopgrid                  - stop the currently running grid routnine\n"
    )

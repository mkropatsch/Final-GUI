...
#   3) Also adding repeatability for the routine with a certain amount of time given for the routine
#
...
import sys, multiprocessing as mp, time
...
class StageGUI(QMainWindow):
    def __init__(self, simulate: bool = True):
       ...
       # st.addRow("E (abs)", self.lab_e)
        ...
        self.spn_total_s = QSpinBox()
        self.spn_total_s.setRange(0, 24*3600) # 0 .. 24h
        self.spn_total_s.setValue(0)  # 0 = single grid (no repeat)
        row_duration.addWidget(QLabel("Run time (s)"))
        row_duration.addWidget(self.spn_total_s)
        ...
        # NEW: time limit for whole routine (0 = one grid only)
        self._routine_total_s: int = 0
        self._routine_start_time: float = 0.0
        
        # NEW: accumulate XY steps per grid cycle for step-based undo
        self._routine_nx_total: int = 0
        self._routine_ny_total: int = 0
        
        # NEW: store grid parameters so we can rebuild iterator on repeat
        self._routine_cols: int = 0
        self._routine_rows: int = 0
        self._routine_step_nx: int = 0
        self._routine_step_ny: int = 0
        self._routine_serp: bool = True
    ...
    def _on_routine_start(self):
        ...
        try:
            ...
            total_s = int(self.spn_total_s.value())
        except ValueError:
            QMessageBox.warning(self,
                                "Routine", "Please enter valid integers for steps and dwell.")
            return
        ...
        # NEW: Store grid parameters
        self._routine_cols = cols
        self._routine_rows = rows
        self._routine_step_nx = nx
        self._routine_step_ny = ny
        self._routine_serp = self.chk_serp.isChecked()
        
        
        # Build iterator once; routine tick consumes it
        self._routine_iter = iter(self._grid_iter(cols, rows,
                                                  nx, ny, self.chk_serp.isChecked()))
        
        # Store Z routine settings
        self._routine_nz = nz
        self._routine_dwell = dwell_s
        self._routine_total_s = total_s
        self._routine_start_time = time.time()
        
        # Reset per-cycle XY accumulators
        self._routine_nx_total = 0
        self._routine_ny_total = 0
        ...
        self._post_msg(
            ...
            f"ΔZ={nz} steps, dwell={dwell_s}s, total_time={total_s}s."
        )
    ...
    def _on_routine_stop(self):
        ...
        self._routine_start_time = 0.0
        self._routine_nx_total = 0
        self._routine_ny_total = 0

    def _handle_grid_finished(self):
        """Called when we reach the end of the XY grid iterator for a cycle."""
        # Step-based undo back to origin
        undo_nx = -self._routine_nx_total
        undo_ny = -self._routine_ny_total
        
        # Reset accumulators for the next cycle
        self._routine_nx_total = 0
        self._routine_ny_total = 0
        
        if undo_nx !=0 or undo_ny !=0:
            self._post_msg(
                f"Grid cycle finished. Returning to origin via steps: "
                f"ΔX={undo_nx}, ΔY={undo_ny}."
            )
            self.q_gui_to_gantry.put({
                "type": "gantry_cmd",
                "cmd": "move_steps_xy",
            ...
            self._post_msg("Grid cycle finished (no net XY displacement).")
            
        # Check time limit: stop after this cycle if time exceeded
        if self._routine_total_s > 0 and self._routine_start_time > 0.0:
            elapsed = time.time() - self._routine_start_time
            if elapsed >= self._routine_total_s:
                self._post_msg(
                  f"Routine complete (time limit reached: {elapsed: .1f}s ≥ {self._routine_total_s}s)."  
                )
                self._routine_active = False
                self._routine_iter = None
                self._routine_phase = "idle"
                return
        # If we get here, we want to start another grid cycle
        self._post_msg("Starting next grid cycle...")
        
        # Rebuild iterator for the next cycle
        self._routine_iter = iter(self._grid_iter(
            self._routine_cols,
            self._routine_rows,
            self._routine_step_nx,
            self._routine_step_ny,
            self._routine_serp,
        ))
        
        # After undo move, wait a bit before starting Z-down at new cycle
        self._routine_phase = "z_down"
        self._routine_timer.start(1000) # 1s pause; adjust if needed

    def _routine_issue_next_move(self) -> bool:
        ...
        try:
           ...
           # Finished one full grid cycle
           self._handle_grid_finished()
           return False
        
        # Accumulate total steps for this cycle (for step-based undo)
        self._routine_nx_total += nx
        self._routine_ny_total += ny
    ...
    def _routine_tick(self):
        ...
        if nz == 0 or z_step_mm == 0.0 or dwell_s < 0:
            ...
            moved = self._routine_issue_next_move()
            if not moved:
                # Either:
                # - grid cycle finished and _handle_grid_finished() took over, or
                # - routine was stopped (time limit / estop)
                return
            
            # Schedule next XY -only step
            if dwell_s > 0:
        ...
        if self._routine_phase == "z_down":
            ...
            if not moved:
                # Either:
                # grid cycle finished and _handle_grid_finished() scheduled undo/next cycle, or
                # routine was stopped (time limit / estop)
                # In either case, do NOT start a new Z-cycle here
                return
